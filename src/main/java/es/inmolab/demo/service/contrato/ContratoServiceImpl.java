...
public class ContratoServiceImpl implements ContratoService {
    ...
    @Override
    public List<ContratoDto> getContratoByFechaCreacion(Date fechaCreacion) throws ServiceException {
        log.info("[getContratoByFechaCreacion]");
        try {
            List<Contrato> contratos = contratoRepository.findByFechaCreacion(fechaCreacion);
            return contratos.stream()
                            .map(contrato -> modelMapper.map(contrato, ContratoDto.class))
                            .collect(Collectors.toList());
        } catch (DataAccessException e) {
            log.error("Error al recuperar contratos por fecha de creación", e);
            throw new ServiceException("Error al recuperar contratos por fecha de creación", ErrorCode.PROPIEDAD_ERROR_GENERAL);
        }
    }

    @Override
    public List<ContratoDto> getContratoByFechaEntrada(Date fechaEntrada) throws ServiceException {
        log.info("[getContratoByFechaEntrada]");
        try {
            List<Contrato> contratos = contratoRepository.findByFechaEntrada(fechaEntrada);
            return contratos.stream()
                            .map(contrato -> modelMapper.map(contrato, ContratoDto.class))
                            .collect(Collectors.toList());
        } catch (DataAccessException e) {
            log.error("Error al recuperar contratos por fecha de entrada", e);
            throw new ServiceException("Error al recuperar contratos por fecha de entrada", ErrorCode.PROPIEDAD_ERROR_GENERAL);
        }
    }

    @Override
    public List<ContratoDto> getContratoByFechaSalida(Date fechaSalida) throws ServiceException {
        log.info("[getContratoByFechaSalida]");
        try {
            List<Contrato> contratos = contratoRepository.findByFechaSalida(fechaSalida);
            return contratos.stream()
                            .map(contrato -> modelMapper.map(contrato, ContratoDto.class))
                            .collect(Collectors.toList());
        } catch (DataAccessException e) {
            log.error("Error al recuperar contratos por fecha de salida", e);
            throw new ServiceException("Error al recuperar contratos por fecha de salida", ErrorCode.PROPIEDAD_ERROR_GENERAL);
        }
    }
    ...
    public ContratoDto getContratoByNroContrato(long id) throws ServiceException {
        ...
        try {
            Contrato contrato = contratoRepository.findById(id)
                                                  .orElseThrow(() -> new ServiceException("Contrato no encontrado", ErrorCode.PROPIEDAD_NO_ENCONTRADA));
